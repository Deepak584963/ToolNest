        ...
        return (
          <article key={item.question} className="rounded-xl border border-slate-200 bg-white/80">
            ...
            >
              ...
              <span className={`text-lg text-slate-500 transition-transform ${isOpen ? "rotate-180" : ""}`} aria-hidden>
                {isOpen ? "−" : "+"}
              </span>
            </button>
            {isOpen ? <p id={panelId} role="region" aria-labelledby={buttonId} className="px-4 pb-4 text-sm leading-6 text-slate-600">{item.answer}</p> : null}
          </article>
        );
      })}
    </section>
